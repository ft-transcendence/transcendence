...
class Message extends React.Component<Msg, MsgState> {
  constructor(props: Msg) {
    super(props);
    this.state = { showMsg: false, type: 0 };
  }
...
export default class Game extends React.Component<{}, StatePong> {
  ...
  componentDidMount() {
    ...
    socket.on("game_started", () => this.setState({ gameStarted: true }));
    ...
    socket.on("end_game", (winner: number) =>
      winner === this.state.playerNumber
        ? this.setState({ msgType: 2, gameStarted: false })
        : this.setState({ msgType: 3, gameStarted: false })
    );
  }
  ...
  render() {
    ...
    const showBorder = this.state.gameStarted
      ? "2px solid rgb(0, 255, 255)"
      : "0px solid rgb(0, 255, 255)";
    const showShadow = this.state.gameStarted
      ? "0px 0px 5px 5px rgb(80, 200, 255), inset 0px 0px 5px 5px rgb(0, 190, 255)"
      : "0";
    var leftName;
    var rightName;
