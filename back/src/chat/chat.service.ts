...
export class ChatService {
  ...
  organize__channelToJoin(source: any) {
    ...
    if (source.admin)
      for (let i = 0; i < source.admin.length; i++) {
        const channel = source.admin[i].name;
        channels.push(channel);
      }
    if (source.member)
      for (let i = 0; i < source.member.length; i++) {
        const channel = source.member[i].name;
        channels.push(channel);
      }
    if (source.invited)
      for (let i = 0; i < source.invited.length; i++) {
        const channel = source.invited[i].name;
        channels.push(channel);
      }
    return channels;
  }
  ...
  organize__previews(source: any, email: string) {
    ...
    if (source.owner) {
      for (let i = 0; i < source.owner.length; i++) {
        ...
        if (source.owner[i].owners.length > 1) {
          ...
          else dmName = source.owner[i].owners[0].username;
        } else dmName = 'No One';
        const msgCount = source.owner[i].messages.length;
        const element: chatPreview = {
          ...
          lastMsg:
            msgCount > 0 ? source.owner[i].messages[msgCount - 1].msg : '',
    ...
    if (source.admin)
      for (let i = 0; i < source.admin.length; i++) {
        const msgCount = source.admin[i].messages.length;
        const element: chatPreview = {
          ...
          lastMsg:
            msgCount > 0 ? source.admin[i].messages[msgCount - 1].msg : '',
    ...
    if (source.member)
      for (let i = 0; i < source.member.length; i++) {
        const msgCount = source.member[i].messages.length;
        const element: chatPreview = {
          ...
          lastMsg: msgCount > 0 ? source.member[i].messages[0].msg : '',
    ...
    if (source.invited)
      for (let i = 0; i < source.invited.length; i++) {
        const msgCount = source.invited[i].messages.length;
        const element: chatPreview = {
          ...
          lastMsg: source.invited[i].isPassword
            ? ''
            : msgCount > 0
  ...
  organize__onePreview(source: any) {
    let msgCount = 0;
    if (source.messages) msgCount = source.messages.length;
  ...
  async update__muteCheckAt(id: number, channelId: number) {
    try {
      await this.prisma.mute.updateMany({
        where: {
          AND: [{ userId: id }, { cid: channelId }, { finished: false }],
        },
        data: {
          checkAt: new Date(),
        },
